...
void lptoken::transfer(const name &from, const name &to, const asset &quantity,
                       const string &memo) {
...
void lptoken::open(const name &owner, const symbol &symbol,
...
uint64_t lptoken::sub_balance(const name &owner, const asset &value) {
  ...
  if (owner != _self && owner != SWAP_CONTRACT) {
    ...
    if (balance == value.amount) {
      ...
      // auto costs = get_costs(value.symbol.code(), value.amount);
      lps.modify(lpi, owner, [&](auto &a) { a.liquidity -= value.amount; });
    }
  }
  return balance - value.amount;
}

uint64_t lptoken::add_balance(const name &owner, const asset &value,
                              const name &ram_payer) {
  ...
  if (owner != _self && owner != SWAP_CONTRACT) {
    ...
    if (lpi == lps.end()) {
      ...
      lps.modify(lpi, same_payer,
                 [&](auto &a) { a.liquidity += value.amount; });
    }
  }

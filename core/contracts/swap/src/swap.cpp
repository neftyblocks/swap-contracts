...
ACTION swap::createpair(name creator, extended_symbol token0,
  ...
  while (itr != byhash_idx.end() && itr->hash() == hash) {
    check(!is_same_pair(token0, token1, itr->token0, itr->token1),
          "pair already exists");
    itr++;
  }
...
ACTION swap::refund(name owner, uint64_t pair_id) {
  ...
  if (itr->quantity0.amount > 0) {
    transfer_to(p_itr->token0.get_contract(), owner, itr->quantity0,
                std::string("Swap cancel refund"));
  }
  if (itr->quantity1.amount > 0) {
    transfer_to(p_itr->token1.get_contract(), owner, itr->quantity1,
                std::string("Swap cancel refund"));
  }
  deposits.erase(itr);
}
...
void swap::handle_swap(std::vector<uint64_t> ids, name from, name contract,
  ...
  for (uint8_t i = 0; i < ids.size(); i++) {
    ...
    to_asset =
        swap_token(ids[i], from, from_asset.contract, from_asset.quantity);
    if (get_config("mine.status") == 1) {
      action(permission_level{_self, "active"_n}, get_account("mine.account"),
             name("mine"), std::make_tuple(from, pair_id, from_asset, to_asset))
          .send();
    }
  }
...
void swap::remove_liquidity(name owner, uint64_t pair_id, uint64_t amount) {
  ...
  transfer_to(p_itr->token0.get_contract(), owner,
              asset(amount0, p_itr->token0.get_symbol()),
              string("Swap remove liquidity"));
  transfer_to(p_itr->token1.get_contract(), owner,
              asset(amount1, p_itr->token1.get_symbol()),
              string("Swap remove liquidity"));
  ...
  auto data = std::make_tuple(
      pair_id, owner, amount, quantity0, quantity1, p_itr->total_liquidity,
      p_itr->token0.get_contract(), p_itr->token1.get_contract(),
      p_itr->reserve0, p_itr->reserve1);
  action(permission_level{_self, "active"_n}, _self, "logremliq"_n, data)
      .send();
}

void swap::handle_deposit(uint64_t pair_id, name owner, name contract,
                          asset quantity) {
  auto p_itr = _pairs.require_find(pair_id, "pair does not exist");
...
void swap::addliquidity(name owner, extended_symbol token0,
  ...
  if (amount0_refund > 0) {
    transfer_to(p_itr->token0.get_contract(), owner,
                asset(amount0_refund, p_itr->token0.get_symbol()),
                string("Swap deposit refund"));
  }
  if (amount1_refund > 0) {
    transfer_to(p_itr->token1.get_contract(), owner,
                asset(amount1_refund, p_itr->token1.get_symbol()),
                string("Swap deposit refund"));
  }
  ...
  auto quantity0 = asset(amount0, p_itr->token0.get_symbol());
  auto quantity1 = asset(amount1, p_itr->token1.get_symbol());
  auto data2 = std::make_tuple(
      pair_id, owner, liquidity, quantity0, quantity1, p_itr->total_liquidity,
      p_itr->token0.get_contract(), p_itr->token1.get_contract(),
      p_itr->reserve0, p_itr->reserve1);
...
extended_asset swap::swap_token(uint64_t pair_id, name from, name contract,
                                asset quantity) {
  auto p_itr = _pairs.require_find(pair_id, "pair does not exist");
  bool is_token0 = contract == p_itr->token0.get_contract() &&
                   quantity.symbol == p_itr->token0.get_symbol();
  bool is_token1 = contract == p_itr->token1.get_contract() &&
                   quantity.symbol == p_itr->token1.get_symbol();
  ...
  auto data = std::make_tuple(pair_id, from, token_in.get_contract(), quantity,
                              token_out.get_contract(), output.quantity,
                              asset(trade_fee + protocol_fee, quantity.symbol),
                              trade_price, p_itr->total_liquidity,
                              p_itr->reserve0, p_itr->reserve1);
  action(permission_level{_self, "active"_n}, _self, "logswap"_n, data).send();

  if (get_config("orac.status") == 1) {
    auto push_data =
        std::make_tuple(pair_id, reserve0, reserve1, balance0, balance1);
    action(permission_level{_self, "active"_n}, get_account("orac.account"),
           "update"_n, push_data)
        .send();
  }
...
void swap::update_pair(uint64_t pair_id, uint64_t balance0, uint64_t balance1,
...
void swap::lognewpair(uint64_t pair_id, name creator, extended_symbol token0,
                      extended_symbol token1) {
  require_auth(_self);
}

void swap::logaddliq(uint64_t pair_id, name owner, uint64_t liquidity,
                     asset quantity0, asset quantity1, uint64_t total_liquidity,
                     name contract0, name contract1, asset reserve0,
                     asset reserve1) {
  require_auth(_self);
}

void swap::logremliq(uint64_t pair_id, name owner, uint64_t liquidity,
                     asset quantity0, asset quantity1, uint64_t total_liquidity,
                     name contract0, name contract1, asset reserve0,
                     asset reserve1) {
  require_auth(_self);
...
void swap::logswap(uint64_t pair_id, name owner, name contract_in,
